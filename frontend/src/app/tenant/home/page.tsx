...
export default function TenantHomePage() {
  ...
  const wUnits = activeBill.we - activeBill.ws;
  const wCost = wUnits * waterRate;

  const eUnits = activeBill.ee - activeBill.es;
  const eCost = eUnits * elecRate;

  const promptpay = activeBill.promptpay || dorm?.promptpay || "";
    ...
    <>
      <div className="mb5">
        ...
        <div className="pg-sub">
          ประจำเดือน {activeBill.month} {activeBill.year}
        </div>
      </div>
          ...
          <div className="mt4">
            {isUnpaid && (
              <button
                ...
                onClick={() => openModal("m-pay", { billId: activeBill.id, promptpay })}
              >
                แจ้งชำระเงิน
              </button>
            )}
